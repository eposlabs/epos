...
export class PkgParser extends $gl.Unit {
  private utils = new $gl.PkgParserUtils(this)
  ...
  private async parseSrc(bundle: Obj, dir: string) {
    ...
    for (const path of paths) {
      const ext = this.$.libs.path.extname(path).toLowerCase()
  ...
  private async parsePaths(paths: string[], dir: string) {
    ...
    for (const path of paths) {
      ...
      const fullPath = this.$.libs.path.join(dir, path)
