...
class Rebundle {
  ...
  onConfigResolved = async (config: ResolvedConfig) => {
    // Save resolved config
    this.config = config

    // Cleanup output directory if user's `emptyOutDir` is `true`
    if (this.emptyOutDir) {
      await fs.rmdir(this.config.build.outDir, { recursive: true })
    }
  ...
  onGenerateBundle = (output: NormalizedOutputOptions, bundle: OutputBundle) => {
    const prefix = '.'
  ...
  onWriteBundle = async (output: NormalizedOutputOptions, bundle: OutputBundle) => {
    const dir = output.dir
    if (!dir) throw new Error('Output directory is not specified')

    const { bundles, ...common } = this.options
    const chunks = getJsEntryChunks(bundle)
