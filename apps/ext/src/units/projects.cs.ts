...
export class Projects extends cs.Unit {
  ...
  private executeLiteJsFromCookies() {
    ...
    for (const part of document.cookie.split('; ')) {
      ...
      // It is important to add `SameSite=None; Secure;`, otherwise cookie won't be modified in iframes
  ...
  private async injectProjects() {
    ...
    // Inject JS
    const busTabData = await this.$.bus.getTabData()
    const js = await this.$.bus.send<sw.Projects['getJs']>('Projects.getJs', address, busTabData)
